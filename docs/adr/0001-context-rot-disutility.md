...
- Contract invariants:
  ...
  - **Escape hatch:** a flag to emit the full value row (all w ≤ C, not just kinks) for non-monotone consumer utilities. The row is already computed and discarded today; exposure is an API decision, not new computation. *(Implementation note, 2026-08-23: as shipped, the frontier is a standalone exact sweep over dominance-reduced groups — the same mathematical object at one extra O(C·k̄) sweep, not literally the DP's row; and on the divide-and-conquer path no full row ever exists. Full-row escape hatch remains future work (R1).)*
