...
1. A five-stage exact MCKP pipeline in pure TypeScript, zero runtime
   dependencies, every decision an integer cross-product (§4–§5).
2. A separation of two reduction notions the informal literature tends to
   ...
   *upper convex hull* (bounds-only — feeds the LP), with a worked example
   where conflating them returns a provably wrong answer (§5.4).
3. Three propositions with proofs: dominance soundness, LP-integrality as
   ...
   argument), and fathom-bound validity (§5).
4. Empirical validation: 600-seed adversarial brute-force cross-check and
   measured latencies on production-shaped instances (§6).
...
### 4.2 Stage 1 — Pareto reduction (exact)
...
increasing, profit strictly increasing. Soundness is Proposition 1 (§5.1).
...
### 4.3 Stage 2 — Convex hull (bounds only)
...
be integral-optimal — §5.4's worked example shows a dent beating both hull
...
### 4.4 Stage 3 — LP relaxation (integer walk)
...
4. Else: record the *break* on first occurrence — fractional fill
   ...
   skip-and-continue rule is Proposition 2b (§5.2b).
5. Iterate until no open segments remain. If no break ever occurred, the LP
   solution is integral — the certificate of Proposition 2 (§5.2) fires and
...
incumbent quality is high; §6.2 reports DP-invocation rates.

### 4.5 Stage 4 — Fathoming (λ_max bound)
...
monotonicity (Proposition 3, §5.3). We use the uniform `λmax` rather than
...
### 4.6 Stage 5 — Exact DP (windowed two-row Bellman)
...
### 4.7 Orchestration and complexity
...
| stage | cost |
|---|---|
| Pareto + hull | `O(Σ k_i log k_i)` |
| LP walk | `O((Σ k_i)²)` worst, tiny constants at `k ≤ 6` |
| fathom | `O(Σ k_i)` |
| DP | `O(C · n · k̄)` time, `O(C)` space + `O(C·n)` back-pointers |

DP-dominated at the design regime; back-pointer memory is the v0.2 target
for OR-Tools' re-solve-on-residual trick to reclaim.
...
"skip-and-continue" is exactly where naive adaptations go wrong — §6.1's
fuzz styles target precisely these seams.

### 5.1 Proposition 1 — Pareto dominance soundness
...
### 5.2 Proposition 2 — LP-integrality certificate
...
entirely. Measured fire rate: 3–97% of instances by shape (§6.2).

### 5.2b Proposition 2b — Dantzig bound validity under skip-and-continue
...
### 5.3 Proposition 3 — Fathom bound validity
...
### 5.4 The dent example (hull ≠ integral optimum)
...
twice and compare full serialized outputs (§6.1).

## 6. Empirical validation

### 6.1 Correctness — adversarial cross-check against brute force
